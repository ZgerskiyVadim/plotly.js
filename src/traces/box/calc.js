            ...
            /*
                ...
                case 'method3':
                    if(N % 2) {
                        ...
                        // N.B. the indices use to index into the sample array
                        //  are of (-1) of the ones listed on
                        //  https://en.wikipedia.org/wiki/Quartile
                        //  which assumes indexing starting at 1 (not 0 as in JS)

                        if(nth % 1) {
                            nth = (N - 3) / 4;
                            cdi.q1 = 0.75 * boxVals[nth] + 0.25 * boxVals[nth + 1];
                            cdi.q3 = 0.25 * boxVals[3 * nth + 1] + 0.75 * boxVals[3 * nth + 2];
                        } else {
                            cdi.q1 = 0.25 * boxVals[nth - 1] + 0.75 * boxVals[nth];
                            cdi.q3 = 0.75 * boxVals[3 * nth] + 0.25 * boxVals[3 * nth + 1];
                        }
                    } else {
                        lower = boxVals.slice(0, N / 2);
                        upper = boxVals.slice(N / 2);
